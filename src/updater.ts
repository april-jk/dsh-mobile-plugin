...
type ReleaseResponse = { tag_name?: unknown; html_url?: unknown; body?: unknown };
...
type Runner = (tag: string) => Promise<void>;
...
function defaultRunner(tag: string) {
...
export class PluginUpdater {
  ...
  constructor(options: { run?: Runner; fetcher?: Fetcher } = {}) {
    this.run = options.run ?? defaultRunner;
    this.get = options.fetcher ?? fetch;
  }

  state(): UpdateStatus { return { ...this.status }; }

  async check(force = false) {
    if (!force && Date.now() - this.checkedAt < CHECK_INTERVAL_MS && this.status.latestVersion) return this.state();
  ...
  private async fetchLatest() {
    try {
      ...
      if (!validTag(release.tag_name)) throw new Error("invalid_release_version");
  ...
  async update() {
    ...
    if (!latest.updateAvailable || !latest.latestVersion || this.status.updating) return this.state();
