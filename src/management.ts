...
function filePath(value: string | null) {
...
async function handleFiles(req: IncomingMessage, res: ServerResponse) {
  ...
  if (url.searchParams.get("download") === "1") {
    if (!info.isFile() || info.size > MAX_PREVIEW_BYTES) return json(res, 413, { reason: "preview_too_large" });
  ...
  if (info.isDirectory()) {
    ...
    const items = await Promise.all(entries.sort((a, b) => a.name.localeCompare(b.name)).slice(0, 500).map(async entry => {
      const child = resolve(path, entry.name); const childInfo = await stat(child);
  ...
  if (/\.(png|jpe?g|gif|webp|bmp)$/.test(lower)) return json(res, 200, { kind: "image", path: displayPath });
